...
CREATE TABLE IF NOT EXISTS eval_scores (
  ...
  -- revision tracking (self-referential chain for DPO preference pairs)
  revision_number   INT NOT NULL DEFAULT 0,
  previous_eval_id  UUID REFERENCES eval_scores(id),

  -- which model performed the evaluation
  eval_model VARCHAR(100),

  -- calibration quality flag (see migration 014)
  calibration_void BOOLEAN NOT NULL DEFAULT false,
...
COMMENT ON TABLE eval_scores IS 'Multi-dimensional quality scores for agent task outputs. Used as DPO training signal.';
COMMENT ON COLUMN eval_scores.calibration_void IS 'true = score produced by broken eval path; exclude from DPO training and calibration.';

-- =============================================================================
-- 014_flag_void_eval_scores.sql
-- =============================================================================
-- Column already included above in 013 definition.
-- The UPDATE below (run at migration time) marks pre-fix 3.0 fallback scores.
-- In schema.sql this is a no-op after a fresh install; it only matters when
-- applied incrementally to a DB that already has pre-fix rows.
